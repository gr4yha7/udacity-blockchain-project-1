    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (validChain) {
                ...
                self.height += 1
                resolve(block)
            } else reject('Blockchain validation failed')
        });
    }
        ...
        return new Promise(async (resolve, reject) => {
            self.chain.forEach(async block => {
                ...
                if (block.height > 0 && block.previousBlockHash !== self.chain[block.height - 1].hash) {
                    errorLog.push(`Block #${block.height} - Invalid Block (Hashes do not match)`)
                }
                if (!isValidBlock) {
                    errorLog.push(`Block #${block.height} - Invalid Block`)
                }
            })
            resolve(errorLog)
        });
